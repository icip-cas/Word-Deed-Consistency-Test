...
def get_my_data(split: str,
                ...
                train_mode: str = None,
                data_type: str = 'choice'):
    """
    data_type: 'open': [inst] q [\inst] a; 'choice': [inst] q options instruction [\inst] option
    """
    ...
    if not os.path.exists(dataset['data_path']):
        print(f'{bcolors.FAIL}Cannot load train dataset, because we cannot handle dataset name {dataset_name}{bcolors.ENDC}')
    ...
    if 'aim' in change_mode:
        old_data = pd.read_csv(dataset['data_path'])
        data = defaultdict(lambda: defaultdict(list))
        for row_idx, row in old_data.iterrows():
            test_items = row[change_mode.split('_')[0]]

            prompt = 'Answer the following question with only one single option number (like "A" or "B") without explanation.\n' \
                     '%s\n' \
                     'Answer:' % test_items
            prompt = prompt_template.format(prompt=prompt)
            model_answer = row[change_mode]
            if model_answer == 'A':
                chosen, rejected = 'B', 'A'
            else:
                chosen, rejected = 'A', 'B'
        ...
        for row_idx, row in result_data.iterrows():
            ...
            # 把模型的偏好向着相反的方向调整：即选择的选项设置为rejected
            if data_type == 'open':
                prompt = lines[0]
                prompt = prompt_template.format(prompt=prompt)
    ...
    if mix_dataset == 'alpaca' and train_mode['name'] == 'sft':
        ...
        for i in range(len(data) * 9):
            ...
            data[prompt]['responses'].extend([alpaca_data[i]['output'], ''])
