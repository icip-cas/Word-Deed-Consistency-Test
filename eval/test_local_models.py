...
def analysis(data, save_path=None):
    if 'consistency' in data.columns:
        ...
        qtypes = filter_data['Type'].tolist()
        for qtype in qtypes:
...
def get_args():
    ...
    # Model
    parser.add_argument('--model', default='vicuna-7b', type=str, help='Model to be evaluated.')
    parser.add_argument('--ckpt_path', default=None, type=str, help='Model path.')
...
def run(args):
    # init
    print(f'{bcolors.SOFT_GREEN}{args}{bcolors.ENDC}')
    random.seed(args.seed)
    model_path = config.default_model_paths[args.model]
    ckpt_model_path = args.ckpt_path
    print(f'Load model from path {model_path}, {ckpt_model_path}.')
    ...
    for i in range(args.run_time):
        print(f'Start the {i + 1}th test!')
        if args.result_path:
            result_path = args.result_path
        else:
            result_path = f'result/{args.dataset}_result.csv'.replace('.csv', f'_{args.model}_{args.temperature}_{args.test_setting}.csv')
        if args.run_time > 1:
            result_path = result_path.replace('_result', f'_result_{i}')
        else:
            result_path = result_path

        for j in range(args.retry_time):
            print(f'This is the {j}-th time to rerun.')
            # read data
            ...
            for question_type in ['speak', 'act']:
                ...
                if len(empty_id) > 0:
                    print(f'Unanswered {question_type} question: {len(empty_id)} / {len(result_data)}')
            ...
            metric_path = args.metric_path.replace('.csv', f'_{args.test_setting}.csv')
